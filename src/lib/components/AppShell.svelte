<script lang="ts">
	 ...
	 * z-order to manage.
	 *
	 * Lives here (rather than directly in `src/routes/+page.svelte`) so both
	 * the root route and `/taxon/[taxonid]` (a direct, shareable link to one
	 * species, e.g. for the "Access the code"/report-sharing use case) can
	 * render the exact same app, the latter just pre-selecting a species via
	 * `initialTaxonID` once the index has loaded.
	 */
	...
	interface Props {
		/** Pre-select this species (by AphiaID/taxonID) once the species index has loaded — used by `/taxon/[taxonid]`. */
		initialTaxonID?: number | null;
		/**
		 * Show the "Welcome" modal on mount. Default `true`, matching the
		 * original Shiny app's every-load behaviour — but `/taxon/[taxonid]`
		 * opts out: arriving via a direct link to one species is a deliberate
		 * destination, not a fresh visit to the app as a whole, so it shouldn't
		 * be interrupted by a generic welcome popup.
		 */
		showWelcome?: boolean;
	}
	let { initialTaxonID = null, showWelcome = true }: Props = $props();
	...
	// dismissal/"don't show again" persistence there either) — except when
	// `showWelcome` is false (see the prop doc above).
	function initialWelcomeOpen() {
		return showWelcome;
	}
	...
	// route-provided taxonID exactly once, same as a manual search+select.
	let appliedInitialTaxonID = false;
	$effect(() => {
		if (appliedInitialTaxonID || initialTaxonID == null) return;
		if (!speciesView.index.length) return;
		appliedInitialTaxonID = true;
		activeTab = 'species';
		speciesView.selectSpecies(initialTaxonID);
	});
	...
	$effect(() => {
		...
		fetchTitilerBandStats(PUBLIC_TITILER_URL, url, 1)
			...
			.catch((e) => console.error('[AppShell] Failed to load habitat statistics:', e));
	});
...
<svelte:head>
	<title>{activeTab === 'species' && speciesView.species ? `${speciesView.species.scientificName} — MPA Europe SDM Maps` : 'MPA Europe — SDM Maps'}</title>
</svelte:head>
